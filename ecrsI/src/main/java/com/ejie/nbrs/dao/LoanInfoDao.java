...
public interface LoanInfoDao {
  ...
  /**
   * 
   * searchAllNoPur:查询所有没有 贷款用途 的订单流水号(后台更新一期存量数据防范)
   * 
   * @return
   * @throws
   */
  List<LoanInfo> searchAllNoPur();

  /**
   * 
   * updateDataBySer:插入贷款用途根据订单流水号(后台更新一期存量数据防范)
   * 
   * @param loaninfo
   * @return
   * @throws
   */
  int updateDataBySer(LoanInfo loaninfo);
   ...
   * @return
   */
  int insertBaseLoanInfo(LoanInfo loaninfo);
   ...
   *        aBaseName：贷款客户姓名；eProduct：贷款产品； status：审核结果； nodeStatus：节点标记码；flowStatus：流程状态码；
   * @param @return 参数
   ...
   * @Description: TODO 根据不同的节点码、状态码返回不同数据
   * @param @param sc currentPage：查询页数；pageSize：每页显示条数； startTime：查询起始时间；endTime：查询结束时间；
   *        aBaseName：贷款客户姓名；eProduct：贷款产品； status：审核结果；flowStatus：流程状态码；
   * @param @return 参数
   * @return List<LoaninfoIndex> 返回类型
   * @throws
   */
