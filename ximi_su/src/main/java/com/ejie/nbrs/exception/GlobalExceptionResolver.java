package com.ejie.nbrs.exception;
...
public class GlobalExceptionResolver implements HandlerExceptionResolver {
  ...
  public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response,
    ...
    // 跳转到定制化的错误页面
    /*
     * ModelAndView error = new ModelAndView("error"); error.addObject("exMsg", ex.getMessage());
     * error.addObject("exType", ex.getClass().getSimpleName().replace("\"", "'"));
     */
    // 返回json格式的错误信息
    try {
      PrintWriter writer = response.getWriter();
      BaseResult<String> result = new BaseResult(1, ex.getMessage());
