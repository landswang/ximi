...
/**
 * ClassName:ImageCode Java验证码案例（基于springMVC方式）
 * 
 * 验证码工具类
 * 
 * @author torreswang
 * @version
 * @since Ver 1.1
 * @Date 2017年7月14日 下午1:50:56
 * @see
 */
@Component("imageCodeeee")
public class ImageCode {
  /*
   * Spring 3支持@value注解的方式获取properties文件中的配置值，大简化了读取配置文件的代码。
   * 在applicationContext.xml文件中配置properties文件,在bean中使用@value注解获取配置文件的值 即使给变量赋了初值也会以配置文件的值为准。
   */
  ...
  public BufferedImage getImage(HttpServletRequest request) {
    ...
    cache.putCacheWithExpireTime(cache_key, sRand, RedisCache.CAHCETIME);
    LOG.info("-----------put cache with key:" + cache_key + " sRand is " + sRand);
