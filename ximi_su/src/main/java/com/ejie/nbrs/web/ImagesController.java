...
public class ImagesController {
   ...
   * 
   * @param @param sc loanId:贷款编号； SerialNo:业务流水号； LoanVoucher:确定的图片数组字符串 eProduct:二级分类/图片所属类别码
   * @param @return 设定文件
   ...
   * 
   * @param @param sc serialNo：流水号；eProduct：所属类别；
   * @param @return 设定文件
  ...
  /**
   * 解析上传的文件 aBaseName:保存文件地址；eProduct：上传图片所属类别区分；
   * 
   * @param file上传的文件
   * @return 文件信息
   * @throws IOException
  ...
  public String photoUpload(@Param("file") MultipartFile file, HttpServletRequest request,
      SearchCondition sc) throws IllegalStateException, IOException, BusinessException {
   ...
   * @param @param sc 必要字段：loanId、serialNo、loanVoucher,realPath
   * @param @return 参数
   * @return String 返回类型
   * @throws
   */
